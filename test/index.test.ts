...
describe('main', () => {
  ...
  it('should work', async () => {
    ...
    const doc2 = path.join(testTools.cwd, '知识库TEST1/Title2_003/文档2_004.md')
    ...
    let doc1Data = readFileSync(path.join(testTools.cwd, '知识库TEST1/Title1_001/文档1_002.md')).toString()
    doc1Data = doc1Data.replace(/\.\/img.*?-(\d{6})\./g, (match, random) => {
      return match.replace(random, '123456')
    })
    expect(doc1Data).toMatchSnapshot()
    const imgDir = path.join(testTools.cwd, '知识库TEST1/Title1_001/img/002')
    const imgList = readdirSync(imgDir)
    const img1 = path.join(testTools.cwd, `知识库TEST1/Title1_001/img/002/${imgList[0]}`)
    expect(readFileSync(img1).length).toBe(99892)
    const img2 = path.join(testTools.cwd, `知识库TEST1/Title1_001/img/002/${imgList[1]}`)
